    ...
    <section id="method" className="relative py-32 px-6" aria-labelledby="method-title">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="relative">
          <div className="absolute left-[31px] top-8 bottom-8 w-px hidden md:block" style={{ background: 'linear-gradient(to bottom, #3D7CFF, rgba(61,124,255,0.15), transparent)' }} aria-hidden="true" />
          <div className="flex flex-col gap-12">
            {([0, 1, 2, 3] as const).map((i) => (
              <div key={i} className="flex gap-8 items-start group">
                <div className="flex-shrink-0 w-16 h-16 rounded-full border-2 border-blue flex items-center justify-center font-mono font-bold text-blue text-lg transition-all duration-300 group-hover:shadow-[0_0_20px_rgba(61,124,255,0.3)]" style={{ backgroundColor: '#04081C' }} aria-hidden="true">
                  {t(`steps.${i}.n`)}
                </div>
                <div className="pt-3">
