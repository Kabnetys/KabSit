    ...
    <section id="services" className="relative py-32 px-6" aria-labelledby="services-title">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-20">
          ...
          <h2 id="services-title" className="mt-4 font-display font-bold text-4xl md:text-5xl text-white">{t('title')}</h2>
        </div>
        <div className="grid md:grid-cols-3 gap-6">
          {([0, 1, 2] as const).map((i) => {
            const Icon = ICONS[i] ?? CpuIcon;
            return (
              <article key={i} className="group relative p-8 rounded-2xl border border-white/10 hover:border-blue/50 transition-all duration-500 hover:shadow-[0_0_40px_rgba(0,102,255,0.15)]" style={{ backgroundColor: 'rgba(10,18,69,0.6)', backdropFilter: 'blur(12px)' }}>
                <div className="text-cyan mb-6 transition-transform duration-300 group-hover:scale-110"><Icon /></div>
                <h3 className="font-display font-semibold text-xl text-white mb-3">{t(`items.${i}.title`)}</h3>
                <p className="text-sm text-white/60 leading-relaxed">{t(`items.${i}.desc`)}</p>
                <div className="absolute top-0 right-0 w-16 h-16 rounded-2xl overflow-hidden pointer-events-none" aria-hidden="true">
